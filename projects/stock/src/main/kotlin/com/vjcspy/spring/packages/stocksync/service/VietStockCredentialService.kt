...
class VietStockCredentialService {
    ...
    fun retrieveCredentials(): VietStockCredential =
        runBlocking {
            ...
            val credentials =
                loggedCrds["cookies"]?.let { cookies ->
    ...
    private suspend fun loggedCookies(): Map<String, String> {
        return try {
            ...
            val response =
                client.post("https://finance.vietstock.vn/Account/Login") {
                    header("Accept", "*/*")
    ...
    private suspend fun retrieveCookiesAndCsrf(
        initCookies: String?,
        needCookie: Boolean,
    ): Map<String, String>? =
        try {
            val response =
                client.get("https://finance.vietstock.vn/doanh-nghiep-a-z/danh-sach-niem-yet?page=1") {
                    header(
                        "Accept",
            ...
            val csrf =
                doc
                    .select("[name=__RequestVerificationToken]")
                    .firstOrNull()
                    ?.attr("value")
                    ?: ""
    ...
    private fun parseCookies(rawCookies: List<String>): String =
        rawCookies.joinToString("; ") {
            it.split(";", limit = 2)[0]
        }
}
