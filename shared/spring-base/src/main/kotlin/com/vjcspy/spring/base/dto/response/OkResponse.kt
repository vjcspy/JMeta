...
data class OkResponse<T>(
    ...
    companion object {
        fun <T> of(data: T): OkResponse<T> = OkResponse(
            data = data
        )
    }

    fun withMetadata(key: String, value: Any): OkResponse<T> = copy(
 ...
 */
fun <T> OkResponse<T>.toResponseEntity(): ResponseEntity<OkResponse<T>> =
